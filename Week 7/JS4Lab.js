...
let myName = "Emily"
let myFriendsName = "Josh"

console.log(eat, drink, myName, myFriendsName);

//Question: Name a reason why you would use const instead of let.
//Answer: const cannot be changed later in the code so you may want to use it for a variable that will never change;
//whereas let can change so if this variable may change over time, you'd want to use let. Additionally, let is typically used
//inside of blocks and not globally. 
...
const sentence1 = `${myName} and ${myFriendsName} are enjoying some ${eat} and ${drink} together.`
...
let sentence = myName + " and " + myFriendsName + " are enjoying some " + eat + " and " + drink + " together."
...
//const drink = "diet coke"

//Question: Read the error message - in your own words - what does it mean?
//Answer: Since we used a const variable to declare the drink, we can't change it. 
//It's already been delcared so we'd have to change it at the first declaration.
...
const lunchTime = (foodChoice, drinkChoice) => {
   return `${myName} and ${myFriendsName} are enjoying some ${foodChoice} and ${drinkChoice} together.`
}
console.log(lunchTime('Pizza','Water'));
...
console.log(foodForTodayAndTomorrow('meatloaf', 'water', lunchTime("Jimmy Johns", "Lemonade")));
